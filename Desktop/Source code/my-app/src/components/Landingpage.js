...
import HeroSection_Images from "./HeroSection_Images";
...
const LandingPage = () => {
  ...
  const handleLoginClick = (e) => {
    ...
    if (token) {
      // Nếu đã có token (đã đăng nhập) -> chuyển đến trang sản phẩm
      navigate('/product');
    } else {
    ...
    <div className="min-h-screen bg-white to-purple-100 text-purple-900">
      ...
      <header className="container mx-auto px-4 py-6 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <img src="/images/logo.png" alt="logo" className="w-20 h-20 rounded-lg" />
          <motion.div
          ...
          <ul className="flex space-x-6">
            ...
            <li>
              <Link
                ...
                className="hover:text-purple-300 text-lg font-bold italic"
              >
                Đăng nhập
              </Link>
            </li>
      ...
      <section className="py-20 bg-purple-800 ">
        <div className="container mx-auto px-4 text-center">
          ...
          >
            Liên hệ ngay hôm nay và trở thành một phần của mạng lưới đại lý thành công của chúng tôi!
          </motion.p>

          <Link to="https://zalo.me/0925360299">
            <motion.button
              whileHover={{ scale: 1.1, boxShadow: "0px 0px 20px rgba(255, 255, 255, 0.4)" }}
              whileTap={{ scale: 0.95 }}
              className="bg-white text-purple-900 font-bold py-3 px-8 rounded-full text-lg transition duration-300 italic"
            >
              Liên hệ ngay
            </motion.button>
          </Link>
